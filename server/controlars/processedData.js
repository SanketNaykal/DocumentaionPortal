...
export const setNewBlogCode = async (req, res) => {
    try{
        const q1 = 'SELECT MAX(`idpost`) FROM `ameyzingengineer(lms)`.post;';
        const [lastID] = await db.query(q1);        
        if(lastID.length){
            ...
            if(inheritedID[0] === '00'){
                ...
                const q2 = 'UPDATE `ameyzingengineer(lms)`.`post` SET `code` = ? WHERE `idpost` = ?;';
                const [result] = await db.query(q2, [blogCode, currentBlogId]);
                ...
                const [result2] = await db.query(q2, [blogCode2, inheritedID[0]]);
